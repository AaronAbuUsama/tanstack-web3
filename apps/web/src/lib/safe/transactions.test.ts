import { describe, it, expect } from 'vitest'
import { buildTransaction, buildContractCall, buildBatchTransactions, encodeContractCall, OperationType } from './transactions'

describe('OperationType', () => {
  it('has Call as 0', () => {
    expect(OperationType.Call).toBe(0)
  })
  it('has DelegateCall as 1', () => {
    expect(OperationType.DelegateCall).toBe(1)
  })
})

describe('buildTransaction', () => {
  it('builds with defaults', () => {
    const tx = buildTransaction({ to: '0x0000000000000000000000000000000000000001' })
    expect(tx.to).toBe('0x0000000000000000000000000000000000000001')
    expect(tx.value).toBe('0')
    expect(tx.data).toBe('0x')
    expect(tx.operation).toBe(OperationType.Call)
  })

  it('converts ETH value to wei', () => {
    const tx = buildTransaction({ to: '0x0000000000000000000000000000000000000001', value: '1' })
    expect(tx.value).toBe('1000000000000000000')
  })

  it('passes custom data', () => {
    const tx = buildTransaction({ to: '0x0000000000000000000000000000000000000001', data: '0xdeadbeef' })
    expect(tx.data).toBe('0xdeadbeef')
  })
})

describe('buildBatchTransactions', () => {
  it('builds multiple transactions', () => {
    const txs = buildBatchTransactions({
      transactions: [
        { to: '0x0000000000000000000000000000000000000001' },
        { to: '0x0000000000000000000000000000000000000002', value: '0.5' },
      ],
    })
    expect(txs).toHaveLength(2)
    expect(txs[0].to).toBe('0x0000000000000000000000000000000000000001')
    expect(txs[1].value).toBe('500000000000000000')
  })
})

describe('buildContractCall', () => {
  it('encodes function call data', () => {
    const abi = [{ type: 'function', name: 'increment', inputs: [], outputs: [], stateMutability: 'nonpayable' }] as const
    const tx = buildContractCall({ to: '0x0000000000000000000000000000000000000001', abi, functionName: 'increment' })
    expect(tx.data).toBeTruthy()
    expect(tx.data).not.toBe('0x')
    expect(tx.operation).toBe(OperationType.Call)
  })
})

describe('encodeContractCall', () => {
  it('returns encoded data', () => {
    const abi = [{ type: 'function', name: 'increment', inputs: [], outputs: [], stateMutability: 'nonpayable' }] as const
    const data = encodeContractCall(abi, 'increment')
    expect(data.startsWith('0x')).toBe(true)
  })
})
