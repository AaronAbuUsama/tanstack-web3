...
describe('useTransactions', () => {
  ...
  it('submits confirmations through Safe Transaction Service', async () => {
    vi.mocked(isTxServiceSupportedChain).mockReturnValue(true)
    vi.mocked(getServicePendingTransactions).mockResolvedValue([mockServiceTx([])])
  ...
  it('enforces execute gating from service confirmations', async () => {
    vi.mocked(isTxServiceSupportedChain).mockReturnValue(true)
    vi.mocked(getServicePendingTransactions).mockResolvedValue([mockServiceTx(['0xownera'])])
    vi.mocked(getServiceTransaction).mockResolvedValue(mockServiceTx(['0xownera']))
    ...
    vi.mocked(getServiceTransaction).mockResolvedValue(mockServiceTx(['0xownera', '0xownerb']))
  ...
  it('keeps local fallback honest for threshold > 1', async () => {
    ...
    const getOwnersWhoApprovedTx = vi.fn()
    ...
    expect(result.current.pendingTxs).toHaveLength(1)
    expect(result.current.pendingTxs[0].confirmations).toBeGreaterThanOrEqual(0)
